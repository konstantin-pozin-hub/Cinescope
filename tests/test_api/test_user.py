...
class TestUser:
    ...
    def test_create_user(self, super_admin, creation_user_data):
        ...
        user_data['roles'] = [role.value for role in creation_user_data['roles']]
        response = super_admin.api.user_api.create_user(creation_user_data).json()

        assert response.get('id'), "ID должен присутствовать"
        assert response['email'] == user_data['email']
        assert response['fullName'] == user_data['fullName']
        assert response['roles'] == user_data['roles']
    ...
    def test_get_user_by_locator(self, super_admin, creation_user_data):
        ...
        assert response_by_id.get('id'), "ID должен присутствовать"
        assert response_by_id['email'] == user_data['email']
        assert response_by_id['fullName'] == user_data['fullName']
        assert len(response_by_id.get('roles', [])) == 1
        assert response_by_id['roles'] == user_data['roles']
    ...
    @pytest.mark.slow
    def test_duble_create_user(self, super_admin, creation_user_data):
        create_user = super_admin.api.user_api.create_user(creation_user_data)
        user_id = create_user.json().get("id")
        assert user_id is not None, "Идентификатор не найден в ответе"
        create_user2 = super_admin.api.user_api.create_user(creation_user_data, expected_status=409)
    ...
    def test_change_user(self, super_admin, creation_user_data, test_user_for_updata):
        create_user = super_admin.api.user_api.create_user(creation_user_data)
        user_id = create_user.json().get("id")
        assert user_id is not None, "Идентификатор не найден в ответе"
        patch_user = super_admin.api.user_api.patch_user(user_id, test_user_for_updata)

    def test_wrong_change_user(self, super_admin, creation_user_data, test_user_for_updata):
        create_user = super_admin.api.user_api.create_user(creation_user_data)
        user_id = create_user.json().get("id")
        assert user_id is not None, "Идентификатор не найден в ответе"
